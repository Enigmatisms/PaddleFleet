...
from paddlefleet.context_parallel_utils import preprocess_index_dual_chunks
...
def block_order(cp_size, rank, backward):
    ...
    (the kernel's ``hier_seqlen_id``). Under DualChunkSwap the local chunk of rank
    ``r`` is the natural block pair ``(r, 2*cp_size-1-r)``, so one permutation of
    the ``2*cp_size`` blocks expresses the swap layout and the traversal at once.
...
def localize_mask(startend_row_indices, seqlen_local, group, mode):
    ...
    return preprocess_index_dual_chunks(
        ...
        chunk_id_second=2 * cp_size - group.rank - 1,
        seq_blocksize=seq_blocksize,
        max_seqlen_q=seq_blocksize,
    )


def gathered_kv_order(mask, group, backward):
    ...
    return (
        ...
        .index_select(block_order(cp_size, group.rank, backward), axis=2)
...
class OverlappedFlashMaskContextParallel(PyLayer):
    ...
    Only ``query``/``key``/``value``/``startend_row_indices`` are tensor inputs,
    so backward returns exactly three gradients (the mask is stop_gradient).
    """

    @staticmethod
    def forward(
        ctx, query, key, value, startend_row_indices, softmax_scale, mode
    ):
        ...
        output, log_sum_exp = _flash_attn_fwd(
            ...
            key,
            value,
            startend_row_indices=gathered_kv_order(mask, group, False),
            causal=False,
            return_lse=True,
            pack_gqa=False,
    ...
    def backward(ctx, output_grad):
        ...
        flashmask_info = FlashMaskInfoPaddle(
            startend_row_indices=gathered_kv_order(mask, group, True),
            is_causal=False,
        )
        # dK/dV are reduce-scattered inside the kernel, hence already local.
        query_grad, key_grad, value_grad, _ = _flash_attn_bwd(
            query,
            key,
            value,
            output,
...
def overlap_flashmask_attention_cp(
    ...
    if learnable_sink is not None:
        raise NotImplementedError(
            "learnable_sink is not supported in overlapped FlashMask context "
            "parallel."
        )
    assert query.shape[1] % 2 == 0, (
        f"Query sequence length must be divisible by 2 for the DualChunkSwap "
        f"layout. Current query sequence length: {query.shape[1]}"
    )
    return OverlappedFlashMaskContextParallel.apply(
        query, key, value, startend_row_indices, softmax_scale, mode
    )
