...
def _get_csa_indexer_topk_fwd_interface():
    from .kernel.csa_indexer_fwd import csa_indexer_topk_fwd_interface

    return csa_indexer_topk_fwd_interface


def _get_csa_indexer_bwd_interface():
    from .kernel.csa_indexer_bwd import csa_indexer_bwd_interface

    return csa_indexer_bwd_interface


def _get_csa_attn_target_reducesum_interface():
    from .kernel.csa_attn_target import csa_attn_target_reducesum_interface
...
def _validate_topk_and_grad(index_q, topk_indices, grad_scores):
    ...
    if tuple(topk_indices.shape) != tuple(grad_scores.shape):
        raise ValueError(
            f"topk_indices shape {tuple(topk_indices.shape)} must match grad_scores shape {tuple(grad_scores.shape)}"
        )
    if topk_indices.shape[0] != batch or topk_indices.shape[1] != seq_len:
        raise ValueError(
            f"topk_indices shape {tuple(topk_indices.shape)} is incompatible with index_q shape {tuple(index_q.shape)}"
        )
...
def csa_attn_target_reducesum(
    ...
    if len(key_comp_mla.shape) != 4:
        raise ValueError(
            f"key_comp_mla must have shape [B, S_comp, H, D], got {tuple(key_comp_mla.shape)}"
        )
    if len(topk_indices.shape) != 3:
        raise ValueError(
            f"topk_indices must have shape [B, S, topk], got {tuple(topk_indices.shape)}"
