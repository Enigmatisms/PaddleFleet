...
class TestTileLangCSAIndexerKernel(unittest.TestCase):
    ...
    def _run_kernel_fwd_case(self, topk_effective):
        from paddlefleet.tilelang_ops.kernel.csa_indexer_fwd import (
    ...
    def _run_kernel_bwd_case(self, topk_effective):
        from paddlefleet.tilelang_ops.kernel.csa_indexer_bwd import (
...
class TestTileLangCSAIndexerLossGrad(unittest.TestCase):
    ...
    def _assert_attn_target_matches_paddle(
        ...
        from paddlefleet.transformer.csa_attention import _compute_attn_target
        ...
        pd_target = _compute_attn_target(
...
# =========================================================================
# K5 attn_target kernel isolated tests
# =========================================================================
...
class TestCSAAttnTargetReducesum(unittest.TestCase):
    ...
    def _run_and_compare(self, b, sq, sk, np_, hn, topk_eff, ratio, seed=2040):
        ...
        w = paddle.randn([b, sq, h_i]).astype("float32")
        query_mla = paddle.randn([b, sq, np_, hn]).astype("bfloat16").detach()
        # MLA invariant: compressed key is shared across all heads.
        # The kernel exploits this by loading one head's key and broadcasting.
        key_single = paddle.randn([b, sk, 1, hn]).astype("bfloat16")
        key_comp_mla = key_single.expand([b, sk, np_, hn]).contiguous().detach()
    ...
    def test_online_softmax_numerical_stability(self):
        ...
        paddle.seed(7777)
        key_small = paddle.randn([b, 4, 1, hn]).astype("bfloat16") * 0.1
        key_large = paddle.randn([b, 4, 1, hn]).astype("bfloat16") * 1.0
        key_single = paddle.concat([key_small, key_large], axis=1)
        key_comp_mla = key_single.expand([b, sk, np_, hn]).contiguous().detach()
